{{-- modal xoa start --}}
<div class="modal fade" id="xoaPhieuXuat{{ $phieuXuat->id_invoice }}" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <form action="{{ route('xulyphieuxuat') }}" method="POST" enctype="multipart/form-data">
                <div class="modal-body">
                    <p class="small" style="font-size:14px !important;">
                        ...
                        [PX{{ $phieuXuat->id_invoice }}] vĩnh viễn và
                        không thể khôi phục lại, nên cân nhắc
                        trước khi xóa</p>
                </div>
                <input name="maPhieuXuatXoa" type="number" hidden value="{{ $phieuXuat->id_invoice }}" required>
